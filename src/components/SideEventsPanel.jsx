...
const SideEventsPanel = () => {
    const [newsList, setNewsList] = useState([]);

    useEffect(() => {
        const fetchNewsList = async () => {
            try {
                const newsData = await getEvents(); // Fetch news data from API
                setNewsList(newsData.data); // Update state with fetched news list
            } catch (error) {
...
export default SideEventsPanel;
